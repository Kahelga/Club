    ...
    private val dispatcher = object : Dispatcher() {
        ...
        override fun dispatch(request: RecordedRequest): MockResponse {
            ...
            if (request.path == "/users/signin" && request.method == "POST") {
                ...
                val login = body.substringAfter("\"login\":\"").substringBefore("\"")
                val password = body.substringAfter("\"pass\":\"").substringBefore("\"")

                // Проверяем логин и пароль и выбираем соответствующий ответ
                val responseFile = when {
                    login == "user1" && password == "1234" -> "user1.json"
                    login == "user" && password == "pass" -> "user2.json"
                    else -> "error.json"
                }
